...
// Wait for Cloudinary widget to load (adjust the event/selector as needed)
window.cloudinary.on('ready', () => {  // OR similar event provided by Cloudinary
    const nonce = document.querySelector('script[nonce]').getAttribute('nonce'); // Get the nonce applied to your scripts (assumes at least one script with a nonce exists)

    // Inject nonce into dynamically added inline styles generated by the widget. Note this must be done on any inline style and script added into the DOM
    const inlineStyles = document.querySelectorAll('style:not([nonce])');  // Select inline styles without nonce

    inlineStyles.forEach(style => style.setAttribute('nonce', nonce));

     // Inject nonce into dynamically added inline scripts generated by the widget. Note this must be done on any inline style and script added into the DOM
    const inlineScripts = document.querySelectorAll('script:not([nonce])'); // Select inline scripts without nonce

    inlineScripts.forEach(script => script.setAttribute('nonce', nonce));
  });
});
